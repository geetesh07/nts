...
MAX_QUEUED_JOBS = 500  # nts.enqueue will start failing when these many jobs exist in queue.
...
def _check_queue_size(q: Queue):
	max_jobs = cint(nts.conf.max_queued_jobs)
	if not max_jobs:
		return
	...
	if cint(q.count) >= max_jobs:
		...
		nts.throw(
			...
			exc=nts.QueueOverloaded,
			primary_action=primary_action if nts.has_permission("System Health Report") else None,
		)
